export class FormControlData {
  ...
  fill(data) {
    ...
    if (data.paciente["hcp"]) {
      ...
      if (hcp["patologias"]) this.patologias = hcp["patologias"];
      if (hcp["alergias"]) this.alergias = hcp["alergias"];
      if (hcp["bioquimicas"]) this.bioquimicas = hcp["bioquimicas"];
      if (hcp["otros"]) this.hcpOtros = hcp["otros"];
    }
    if (data.paciente["hcf"]) {
      var hcf = data.paciente["hcf"];
      if (hcf["patologias"]) this.hcf_patologias = hcf["patologias"];
    }
    if (data.paciente["objetivos"]) this.objetivos = data.paciente["objetivos"];

    if (data.paciente["habitos"]) {
      var habitos = data.paciente["habitos"];
      if (habitos["ejercicios"]) this.ejercicios = habitos["ejercicios"];

      if (habitos["gustos"]) this.gustos = habitos["gustos"];
      else this.gustos.paciente_id = this.paciente.id;
      ...
      if (habitos["otros"]) this.habitosOtro = habitos["otros"];
      else this.habitosOtro.paciente_id = this.paciente.id;
    }
    /*	VA	*/
    ...
    if (data.va) {
      ...
      if (va.detalleGrasa) this.detalleGrasa = va.detalleGrasa;
      if (va.detalleMusculo) this.detalleMusculo = va.detalleMusculo;
    }
    ...
    if (data.dietas) {
      ...
      /*this._setCurrentDieta(first_dieta);*/	  
	  this.setDieta(first_dieta.dieta_id)
    }
  ...
  getFormPrescripcion(): Prescripcion {
    return this.helpers.clone(this.prescripcion);
  }
  getFormPrescripcion__new(): Prescripcion {
    try {
		var _prescripcion	=	new Prescripcion();
      var filter_dieta = this.dietas.filter(
        x => x.dieta_id === this.current_dieta_id
      );

      if (filter_dieta){
		  _prescripcion	=	this.helpers.clone(filter_dieta[0].prescripcion);
		  
		  /*this.prescripcion.set(_prescripcion);*/
		  /*return _prescripcion;*/
	  }
    } catch (err) {
      console.log("E:getFormPrescripcion(" + err.message + ")");
    }
    /*return this.prescripcion;*/
	/*console.log( _prescripcion );*/
    return _prescripcion;
  }
  getFormPrescripcion__old(): Prescripcion {
    try {
      var current_dieta_id = this.current_dieta_id;
      /*console.log('current_dieta_id: ' + current_dieta_id);*/
      var filter_dieta = this.dietas.filter(
        x => x.dieta_id === current_dieta_id
      );
      if (filter_dieta[0]) {
        /*console.log( filter_dieta[0] );*/
        this.prescripcion.set(filter_dieta[0].prescripcion);
      }
    } catch (err) {
      console.log("E:getFormPrescripcion(" + err.message + ")");
    }
  ...
  getIndexOfDieta(dieta_id) {
    var _index = -1;
    for (var i = 0; i < this.dietas.length; i++) {
      if (dieta_id == this.dietas[i].dieta_id) {
        _index = i;
        i = this.dietas.length;
      }
    }
    return _index;
  }
  setLastValuesFormValoracionAntropometrica(va) {
    ...
    if (va.circunferencia_muneca == null) va.circunferencia_muneca = "";
    this.valoracionAntropometrica.lastCircunferencia_muneca =
      va.circunferencia_muneca;
  }
  setPatronMenu(dieta_id, patron_menu) {
    /*this.patronmenu			=	patron_menu;*/
    var filter_dieta = this.dietas.filter(x => x.dieta_id === dieta_id);
    filter_dieta[0].patron_menu = patron_menu;
  }
  setPatronMenuEjemplos(dieta_id, patron_menu_ejemplos) {
    /*this.patron_menu_ejemplos			=	patron_menu_ejemplos;*/
    var filter_dieta = this.dietas.filter(x => x.dieta_id === dieta_id);
    filter_dieta[0].patron_menu_ejemplos = patron_menu_ejemplos;
  }
  ...
  setCurrentDieta(dieta_id) {
    console.log('setCurrentDieta id', dieta_id);
    var filter_dieta = this.dietas.filter(x => x.dieta_id === dieta_id);
    console.log('filter_dieta', filter_dieta);
  ...
  _setCurrentDieta(dieta) {
    this.current_dieta = dieta;
    this.current_dieta_id = dieta.dieta_id;

    this.prescripcion = new Prescripcion();
    this.patronmenu = [];
    this.patron_menu_ejemplos = [];
    /*		Prescripcion	*/
    if (dieta.prescripcion) {
      var prescripcion = this.helpers.clone(dieta.prescripcion);
      this.setPrescripcion(this.current_dieta_id, prescripcion);
    }
    /*		Patron Menu	*/
    if (dieta.patron_menu) {
      this.patronmenu = this.helpers.clone(dieta.patron_menu);
    }
    /*		Patron Menu	Ejemplos	*/
    if (dieta.patron_menu_ejemplos) {
      this.patron_menu_ejemplos = this.helpers.clone(dieta.patron_menu_ejemplos);
    }
  }

  setPrescripcion(dieta_id, prescripcion) {
    var _index = this.getIndexOfDieta(dieta_id);
    this.dietas[_index].prescripcion = prescripcion;
  }
  ...
  setPrescripcionPatronMenu(dieta_id) {
    ...
    /*console.log('first_dieta');console.log(first_dieta);*/
    ...
    this.current_dieta = first_dieta;
    this.current_dieta_id = first_dieta.dieta_id;
    /*if(first_dieta.prescripcion){
			var prescripcion		=	first_dieta.prescripcion;
			this.prescripcion.set(this.current_dieta_id, prescripcion);
		}*/
    if (first_dieta.patron_menu) {
      this.patronmenu = this.helpers.clone(first_dieta.patron_menu);
    }
    if (first_dieta.patron_menu_ejemplos) {
      this.patron_menu_ejemplos = this.helpers.clone(first_dieta.patron_menu_ejemplos);
    }
  }
	setDieta(dieta_id){
		var dieta_filter=	this.dietas.filter(x => x.dieta_id === dieta_id);
		var dieta_found	=	dieta_filter[0];

		this.prescripcion	=	new Prescripcion();
		this.patronmenu 	=	[];
		this.patron_menu_ejemplos	=	[];

		this.current_dieta		=	dieta_found;
		this.current_dieta_id	=	dieta_found.dieta_id;
		if(dieta_found.prescripcion){
			var prescripcion		=	dieta_found.prescripcion;
			this.prescripcion.set( prescripcion );			
		}
		if( dieta_found.patron_menu ){
			this.patronmenu = this.helpers.clone(dieta_found.patron_menu);
		}
		if( dieta_found.patron_menu_ejemplos ){
			this.patron_menu_ejemplos = this.helpers.clone(dieta_found.patron_menu_ejemplos);
		}
	}
...
export class Persona {
  ...
  setEdad() {
    ...
    if (this.fecha_nac) {
      ...
      if (_anhos > 30) edadPaciente_meses += Math.trunc(_anhos / 30);
    }
...
*/
  ...
  restarSumarAlPesoIdeal(pesoIdeal, esMasculino) {
    var valor = "";
    var valor_porcentaje_10 = pesoIdeal * 0.1; /*	10%	*/
    ...
    if (esMasculino) {
      if (estructura_osea > 10.4) {
        ...
        if (estructura_osea > 9.6) valor = "MEDIANA";
        else {
          valor = "GRANDE";
          pesoIdeal += valor_porcentaje_10;
        }
      ...
      if (estructura_osea > 11) {
        ...
        if (estructura_osea > 10.1) valor = "MEDIANA";
        else {
          valor = "GRANDE";
          pesoIdeal += valor_porcentaje_10;
        }
  ...
  calcularEstructuraOsea() {
    if (!this.circunferencia_muneca) return 0;
    /*
=ESTATURA*100/MUÑECA
*/
    var valor = 0;
    ...
    valor = Math.round(
      (Number(this.estatura) * 100) / Number(this.circunferencia_muneca)
    );
...
export class Rdd {
  ...
  ingesta_calorica_recomendada: number;
...
*/
    ...
    if (this.paciente.genero == "M")
      result =
        ...
        13.75 * current_peso +
        5.003 * _estatura -
        6.755 * this.paciente.edad;
    else {
      var _peso = 9.563 * current_peso;
      if (isNaN(_peso)) _peso = 0;
...
*/
    var variable_msj = 0;
    if (this.paciente.genero == "M") variable_msj = 5;
    else variable_msj = -161;

    var result =
      10 * current_peso +
      6.25 * Number(this.va.estatura) * 100 -
      5 * Number(this.paciente.edad) +
      variable_msj;
    return result;
  }
  getTmbPromedio() {
...
*/
  ...
  getTmbRda() {
    ...
    if (this.paciente.edad <= 1) {
       ...
       */
      value = 108;
      if (this.paciente.edad_meses >= 6) value = 98;
    } else {
      /*
       *Niños		1 - 3	102
       *			4 - 6	90
      ...
      if (this.paciente.edad > 1) {
        ...
        if (this.paciente.edad >= 4) {
          value = 90;
          if (this.paciente.edad >= 7 && this.paciente.edad <= 10) value = 70;
          /*
           *Hombres	11 - 14	55
           *			15 -18	45
           * Mujeres	11 - 14	47
          ...
          if (this.paciente.edad >= 11 && this.paciente.edad <= 14) {
            value = 55;
            if (this.paciente.genero == "F") value = 47;
          }
          if (this.paciente.edad >= 15 && this.paciente.edad <= 18) {
            value = 45;
            if (this.paciente.genero == "F") value = 40;
          }
        }
      }
    }
...
*/
    if (this.paciente.edad < 3 || this.paciente.edad > 18) return value;
    ...
    if (this.paciente.edad < 11) {
      if (this.paciente.genero == "M")
        ...
        value =
          8.365 * current_peso + 130.3 * Number(this.va.estatura) + 414.11;
    } else {
      if (this.paciente.genero == "M")
  ...
  getGastoCaloricoReal() {
    ...
    switch (this.metodo_calculo_gc) {
      ...
      case "benedict":
        result =
          this.getTmbBenedict() * this.factor_actividad_sedentaria +
          this.promedio_gc_diario;
        break;
      case "mifflin":
        result =
          this.getTmbMifflin() * this.factor_actividad_sedentaria +
          this.promedio_gc_diario;
        break;
      case "promedio":
        result =
          this.getTmbPromedio() * this.factor_actividad_sedentaria +
          this.promedio_gc_diario;
        break;
      case "rda":
        result =
          this.getTasaBasal() * Number(this.va.peso) + this.promedio_gc_diario;
        break;
      case "schofield":
...
*/
  ...
  /**/
  getIngestaCaloricaRecomendada_new(dieta) {
    /*	Ingesta calórica Recomendada
	=GASTO_CALORICO_REAL+VARIACION_CALORICA
*/
    var result = this.getGastoCaloricoReal() + dieta.variacion_calorica;
    return result;
  }
  doAnalisis_new(dietas) {
    this.getTasaBasal();
    this.gcr = this.getGastoCaloricoReal();
    var _dieta;
    for (var i in dietas) {
      ...
      _dieta.ingesta_calorica_recomendada = this.getIngestaCaloricaRecomendada_new(
        _dieta
      );
    }
  }
...
export class Dieta {
  ...
  public variacion_calorica: Number = 0;
  public consulta_id: number;
  public ingesta_calorica_recomendada: number;
...
export class Prescripcion {
  ...
  set(prescripcion: Prescripcion) {
    ...
    if (prescripcion.items) {
		this.items = this.helpers.clone(prescripcion.items);
    }else 
		this.prepareItems();
  ...
  prepareItems() {
    this.itemsByDefault = [
      /*id, nombre, slug, ngmodel, cantidad, carbohidratos, proteinas, grasas, kcal*/
...
*/
  constructor(
    ...
    public nombre: string,
    public slug: string,
    public porciones: number = 0,
    public carbohidratos: number = 0,
    public proteinas: number = 0,
    public grasas: number = 0,
    public kcal: number = 0,
    public grupo_alimento_nutricionista_id: number = 0
  ) {
	  this.grupo_alimento_nutricionista_id	=	this.id;
  }
}
export class PatronMenu {
  constructor(
    public tiempo_comida_id: number,
    public grupo_alimento_nutricionista_id: number,
...
export class ManejadorDatos {
  ...
  setExtraInfoAlimentos() {
    ...
    this.extraInfoAlimentos["1"] = {
      slug: "leche-descremada",
      ngmodel: "leche_descremada"
    };
    this.extraInfoAlimentos["2"] = { slug: "leche-2", ngmodel: "leche_2" };
    this.extraInfoAlimentos["3"] = {
      slug: "leche-entera",
      ngmodel: "leche_entera"
    };
...
*/
  validateKey(evt, charCode) {
    if (charCode == 8 || (charCode > 34 && charCode < 41)) return true;

    if (charCode < 45 || charCode > 57) return false;
...
*/
  ...
  soloNumerosRange(evt, min = 0, max = 999.99) {
    var charCode = evt.which ? evt.which : evt.keyCode;
    if (charCode == 8 || (charCode > 36 && charCode < 41)) return true;
    if (charCode == 46) {
      var txt = evt.target.value;
      if (txt.indexOf(".") === -1) {
        return true;
  ...
  soloNumerosNegativePositiveDecimal(
    event: KeyboardEvent,
    text: string
  ): boolean {
  ...
  scrollToForm(isConsulta = false) {
    setTimeout(() => {
      ...
      var top = 180; /*	Nuevo	*/
      if (isConsulta) top = 200; /*	Consulta	*/
      window.scrollTo(0, top);
    }, 100);
  }
  ...
  getHourMinutes(value) {
    let _num = String(value).padStart(4, "0");
    let time_formatted = {
      hour: parseInt(_num.substring(0, 2)),
      minutes: parseInt(_num.substring(2, 4))
    };
